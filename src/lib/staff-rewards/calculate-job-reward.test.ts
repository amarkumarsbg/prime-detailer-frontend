...
describe("getCompanyTargetResults", () => {
  ...
  it("uses the highest achieved tier", () => {
    const results = getCompanyTargetResults({
      jobCards: [],
      invoices: [
  ...
  it("distributes equally among all eligible non-super-admin staff", () => {
    const equalSettings = { ...settings, companyTargetDistributionMode: "DISTRIBUTE_EQUALLY" as const };
    const results = getCompanyTargetResults({
      jobCards: [],
      ...
      activeStaffCount: 3,
      settings: equalSettings,
      year: 2026,
      joiningDate: "2026-02-01",
      evaluationDate: new Date(Date.UTC(2026, 1, 20)),
    });
    expect(results[0].totalReward).toBe(7200);
    expect(results[0].eligibleStaffCount).toBe(2);
    expect(results[0].sharePerStaff).toBe(3600);
  });

  it("distributes role wise using global companyTargetRoleShares", () => {
    const roleWiseSettings = {
      ...settings,
      companyTargetDistributionMode: "DISTRIBUTE_ROLE_WISE" as const,
      companyTargetRoleShares: { MECHANIC: 40, SUPERVISOR: 20 },
    };
    const results = getCompanyTargetResults({
      jobCards: [],
      invoices: [{ grandTotal: 90000, createdAt: "2026-02-15T00:00:00.000Z", status: "PAID" }],
      ...
      activeStaffCount: 3,
      settings: roleWiseSettings,
      year: 2026,
      joiningDate: "2026-02-01",
      staffRole: "MECHANIC",
      evaluationDate: new Date(Date.UTC(2026, 1, 20)),
    });
    // Total 7200, mechanic pool 40% = 2880, 2 mechanics => 1440 each
    expect(results[0].totalReward).toBe(7200);
    expect(results[0].sharePerStaff).toBe(1440);
    expect(results[0].shareForRole).toBe(1440);
  });
